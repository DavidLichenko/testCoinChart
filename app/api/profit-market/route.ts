...
async function fetchForexData(ticker) {
    const response = await fetch(
        `https://api.tiingo.com/tiingo/fx/top?tickers=${ticker}&token=${TIINGO_API_KEY}`,
    );
...
// Fetch IEX Data
async function fetchIEXData(ticker) {
    const response = await fetch(
        `https://api.tiingo.com/iex?tickers=${ticker}&token=${TIINGO_API_KEY}`,
    );
    const data = await response.json();
