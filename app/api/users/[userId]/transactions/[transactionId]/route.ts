...
export async function PUT(
    ...
    try {
        ...
        const updatedTransaction = await prisma.$transaction(async (prisma) => {
            const transaction = await prisma.trade_Transaction.update({
                ...
                data: {
                    ...
                    profit: body.profit,
                    status: body.status,
                    closeIn: body.status === 'CLOSE' ? body.closeIn : null,
                    endAt: body.status === 'CLOSE' ? new Date() : null
                },
            })

            if (body.status === 'CLOSE' && body.profit) {
                await prisma.user.update({
                    where: { id: userId },
                    data: {
                        TotalBalance: {
                            increment: body.profit
                        }
                    }
                })
            }
        ...
        return NextResponse.json({
            ...
            message: body.status === 'CLOSE' ? 'Transaction closed and profit added to balance' : 'Transaction updated successfully'
        })
    } catch (error) {
