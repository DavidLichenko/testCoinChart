    ...
    return (
      <Card className="bg-gray-800 border-gray-700">
        <CardContent className="p-6 text-center">
          ...
          <p className="text-sm text-gray-500">Select a ticker to start trading</p>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <Card className="bg-gray-800 border-gray-700">
      ...
      <CardContent className="space-y-3">
        ...
        <div>
          <Label htmlFor="volume" className="text-xs text-gray-300">Volume</Label>
        ...
        {Number.parseFloat(calculateMargin()) > (balance || 0) && (
          <div className="text-xs text-red-400 bg-red-900/20 p-2 rounded">
            Insufficient balance. Required: ${calculateMargin()}, Available: ${balance || 0}
          </div>
        )}
        <div>
          <Label htmlFor="leverage" className="text-xs text-gray-300">Leverage</Label>
        ...
        <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
          ...
          <CollapsibleContent className="space-y-2 mt-2">
            <div className="flex items-center justify-between">
              <Label htmlFor="take-profit" className="text-xs text-gray-300">Take Profit</Label>
              <Switch id="take-profit" checked={takeProfitEnabled} onCheckedChange={setTakeProfitEnabled} />
            </div>
            {takeProfitEnabled && (<Input value={takeProfit} onChange={(e) => setTakeProfit(e.target.value)} className="bg-gray-700 border-gray-600 text-white h-8" placeholder="Take profit price" />)}
            <div className="flex items-center justify-between">
              <Label htmlFor="stop-loss" className="text-xs text-gray-300">Stop Loss</Label>
              <Switch id="stop-loss" checked={stopLossEnabled} onCheckedChange={setStopLossEnabled} />
            </div>
            {stopLossEnabled && (<Input value={stopLoss} onChange={(e) => setStopLoss(e.target.value)} className="bg-gray-700 border-gray-600 text-white h-8" placeholder="Stop loss price" />)}
          </CollapsibleContent>
        </Collapsible>
